...
| `category` | Category? | 관계. **nil = "미분류"(폴더 미지정)** |
...
- **미분류는 가상 카테고리** (`category == nil`) — 삭제·이름변경 불가, 마이그레이션 불필요.
...
| 삭제 (링크·카테고리) | `AlertPrompt` — 카테고리 삭제 시 소속 링크는 미분류로 이동 |
...
| # | 화면 | 내용 |
|---|---|---|
| ① | 보관함 홈 | 리스트 + 상단 필터 칩(전체·미열람·즐겨찾기·미분류·카테고리들) + `.searchable` 검색 + 아카이브 진입 + 설정 진입 |
