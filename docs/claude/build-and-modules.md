...
프로젝트는 `AppName/`(레거시 xcodeproj)와 `AppName/`(Tuist 기반) **두 축**으로 관리됩니다.
신규 작업은 `AppName/`를 기본으로 사용합니다.

### AppName (xcodeproj)

```bash
# Xcode로 빌드 (권장)
open AppName/AppName.xcodeproj

# CLI 빌드
xcodebuild -project AppName/AppName.xcodeproj -scheme AppName -configuration Debug build

# 테스트 실행
xcodebuild -project AppName/AppName.xcodeproj -scheme AppName test
```
...
`AppName/` 폴더는 Tuist 기반의 별도 모듈화 프로젝트입니다. `AppName/`와 병행하여 관리됩니다.
...
> **경계 정책 — BusinessCard**: MyPage·Home·Community·Activity·Auth 등 명함을 노출하는 Feature는 `BusinessCardPresentation`만 링크해 카드 UI(`BusinessCardFaceView` 등)를 재사용하며, 카드 화면을 자체 구현하지 않는다. 렌더링은 2D SwiftUI로 확정 — 초기 구상이던 RealityKit(3D) 렌더링은 폐기됐고(#1196에서 ARKit·RealityKit 링크 해제) AppName에 RealityKit 참조가 없다.
